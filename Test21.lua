...
gui.Name = "TrainingCombatHUD_V21"
...
local title = makeLabel(panel,"TRAINING  //  V21",14,Enum.Font.GothamBold)
...
for _,button in ipairs({
	...
	visualsButton,skyButton,killSoundButton,fovMinus,fovPlus
}) do
...
local function sameTeam(modelOrPlayer)
	...
	local localTeamName = localTeam and localTeam.Name or LocalPlayer:GetAttribute("Team")
	local localTeamColor = localTeam and localTeam.TeamColor
	local targetTeam
	...
	if typeof(modelOrPlayer) == "Instance" and modelOrPlayer:IsA("Player") then
		...
		local attr = modelOrPlayer:GetAttribute("Team")
		if attr ~= nil and localTeamName ~= nil then
			return tostring(attr) == tostring(localTeamName)
		end
		return false
	end

	targetTeam = getTeamValue(modelOrPlayer)
	if targetTeam == nil then return false end
	if typeof(targetTeam) == "Instance" and localTeam then
		return targetTeam == localTeam
	end
	if localTeamName ~= nil then
		return tostring(targetTeam) == tostring(localTeamName)
	end
	return false
end
		...
		if currentTarget then
			if targetStillValid(currentTarget,camera) then
				...
				local alpha = math.clamp(
					CFG.AimStrength * math.clamp(dt*60, 0.75, 1.35),
					0,
					CFG.MaxAimStep
				)
...
pcall(function() game:GetService("ContentProvider"):PreloadAsync({killSound}) end)
...
local function hookKillDetection(player, character)
	...
	playerConnections[key] = hum.Died:Connect(function()
		...
		local confirmed = (killer == LocalPlayer)
		if not confirmed and typeof(killer) == "string" then
			confirmed = killer == LocalPlayer.Name or killer == tostring(LocalPlayer.UserId)
		end
		if not confirmed and currentTarget and currentTarget.player == player then
			-- Fallback for games that don't expose a creator tag: only fire for the
			-- player we were actively tracking at the moment they died.
			confirmed = true
		end
		if confirmed then playKillSound() end
	end)
end
...
local function disableTrainingEffects()
	for _,name in ipairs({
		...
		if obj and (obj:IsA("PostEffect") or obj:IsA("Atmosphere")) then
			obj.Enabled = false
		end
	end
end
