...
local function GetEggRarity(egg)
    local attr = egg:GetAttribute("Rarity") or egg:GetAttribute("EggType") or egg.Name
    if string.find(string.lower(attr), "divine") then return "Divine", 1 end
    if string.find(string.lower(attr), "eternal") then return "Eternal", 2 end
    if string.find(string.lower(attr), "secret") then return "Secret", 3 end
    return nil, 999
end
    ...
    while State.IsFarming do
        ...
        if not isNight then
            ...
            if State.NightCycleDoneThisNight then
                ...
                local waitSpot = Workspace:FindFirstChild("WaitSpot")
                ...
                if not waitSpot or not safeZone then
                    ...
                    if arrivedWait and State.IsFarming then
                        ...
                        if targetEgg then
                            ...
                            local arrivedEgg = FlyToCFrame(GetTargetCFrame(targetEgg))
                            ...
                            if arrivedEgg and targetEgg.Parent and State.IsFarming then
                                ...
                                if captured and State.IsFarming then
                                    ...
                                    local arrivedSafe = FlyToCFrame(GetTargetCFrame(safeZone))
                                ...
                                State.CurrentStatus = "Egg lost during flight!"
                            end
                        else
                            State.CurrentStatus = "No target rarity found!"
                        end
                    end
...
local function BuildGUI()
    local existing = PlayerGui:FindFirstChild("StealEggGUI") or CoreGui:FindFirstChild("StealEggGUI")
