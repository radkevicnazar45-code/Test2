...
local function BuildGUI()
    ...
    CrownBtn.Size = UDim2.new(1,-24,0,34)
    CrownBtn.Position = UDim2.new(0,12,0,75)
    CrownBtn.BackgroundColor3 = Color3.fromRGB(45,48,59)
    CrownBtn.Text = "👑 CROWN  •  OFF"
    CrownBtn.TextColor3 = Color3.fromRGB(255,255,255)
    CrownBtn.Font = Enum.Font.GothamBold
    CrownBtn.TextSize = 11
    CrownBtn.Parent = visualPanel
    corner(CrownBtn, 9)

    local crownEnabled = false
    CrownBtn.MouseButton1Click:Connect(function()
        crownEnabled = not crownEnabled
        CrownBtn.Text = "👑 CROWN  •  " .. (crownEnabled and "ON" or "OFF")
        CrownBtn.BackgroundColor3 = crownEnabled and Color3.fromRGB(38,154,104) or Color3.fromRGB(45,48,59)
        -- No unverified remote/API is called here.
    end)

    local SkinBtn = Instance.new("TextButton")
    SkinBtn.Size = UDim2.new(1,-24,0,34)
    SkinBtn.Position = UDim2.new(0,12,0,117)
    SkinBtn.BackgroundColor3 = Color3.fromRGB(45,48,59)
    SkinBtn.Text = "🧍 CHANGE SKIN"
    SkinBtn.TextColor3 = Color3.fromRGB(255,255,255)
    SkinBtn.Font = Enum.Font.GothamBold
    SkinBtn.TextSize = 11
    SkinBtn.Parent = visualPanel
    corner(SkinBtn, 9)

    SkinBtn.MouseButton1Click:Connect(function()
        -- The current source has no verified skin-change implementation,
        -- so do not invent a RemoteEvent or silently break the script.
        warn("[StealEgg] Change Skin is not connected: no verified skin API exists in this script.")
    end)
    ...
    for i, col in ipairs(colors) do
        ...
        cBtn.Position = UDim2.new(0, 14 + (i-1)*49, 0, 155)
...
LocalPlayer.CharacterAdded:Connect(function()
    ...
    if not ok then
        warn("[StealEgg] GUI rebuild error:\n" .. tostring(err))
    end
end)
