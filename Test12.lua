...
local function getNPCList()
	-- NPC support intentionally disabled in V11.
	return {}

	-- Legacy code below is unreachable and retained only for compatibility.
	local list = {}
	local seen = {}

	local folder = Workspace:FindFirstChild(CFG.NPCFolderName)

	if folder then
		for _, obj in ipairs(folder:GetDescendants()) do
			if obj:IsA("Model") and not seen[obj] and isValidNPC(obj) then
				seen[obj] = true
	...
	if CFG.AcceptNPCsOutsideFolder then
		for _, obj in ipairs(Workspace:GetDescendants()) do
			if obj:IsA("Model")
				and not seen[obj]
				and isValidNPC(obj) then

				-- Avoid treating random humanoid internals as a target.
				if obj:FindFirstChild("Head")
					or CollectionService:HasTag(obj,"TrainingTarget")
					or obj:GetAttribute("TrainingTarget") == true then
...
print("[TrainingCombatHUD V10 FIXED] Players + NPCs enabled")
